...
public class ItemManager : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		{
			{((int)ItemIds.Box), new ItemData(){uid = (int)ItemIds.Box,name = "상자", desc = "골판지 상자다. 덜컹거리는 것이 안에 무언가 든 것 같다.", icon = icons[(int)ItemIds.Box], anyUse = true, OnUsed = events[(int)ItemIds.Box]} },
			{((int)ItemIds.VideoTape), new ItemData(){uid = (int)ItemIds.VideoTape,name = "카세트테이프", desc = "오래된 카세트 테이프다. 왠지 익숙한 기분이다.", icon = icons[(int)ItemIds.VideoTape], anyUse = false, OnUsed = events[(int)ItemIds.VideoTape]} },
		};
